...
```python
    def _power(self, expression: Power) -> _Result:
        ...
        try:
            with localcontext(Context(prec=precision, traps=[InvalidOperation, DivisionByZero, Overflow, FloatOperation])):
        ...
        return self._result(
            ...
            substituted=(
                f"{child.embedded_substituted} ^ "
                f"({expression.numerator}/{expression.denominator})"
            ),
            reason=(
...
def test_no_match_on_a_non_exhaustive_rule_reports_no_match() -> None:
    rule = DecisionRule(
        ...
        rows=(
            DecisionRow(
                matchers=(Matcher(input="level", op="range", minimum=Decimal(0), maximum=Decimal(1)),),
                values=(DecisionValue(name="protection", categorical="basic"),),
                source=SOURCE,
            ),
        ),
    ...
    for declared in rule.inputs:
        ...
        if declared.kind == "categorical" and value not in declared.allowed_values:
            raise EvaluationError(
                f"input {declared.name!r} is outside its allowed values"
            )
