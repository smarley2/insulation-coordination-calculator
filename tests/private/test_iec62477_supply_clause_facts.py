...
def _placeholder_facts(draft: ImportedRuleDraft) -> dict[str, tuple[SupplyFact, ...]]:
    ...
    A tuple per route rather than one statement, because the completion guard refuses a route that
    leaves a known statement of its clause unauthored -- see ``_system_voltage_placeholders`` for
    the one subclause whose licensed fragment carries several. Every other route's fragment is one
    node, so one placeholder statement still completes it.
    """
