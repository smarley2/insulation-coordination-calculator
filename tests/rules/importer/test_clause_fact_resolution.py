...
def _system_voltage_fact(
    draft: ImportedRuleDraft, *, statement_index: int, node_order: int
) -> SystemVoltageFact:
    return SystemVoltageFact(
        statement_index=statement_index,
        node_references=(_cited(draft, SV_FRAGMENT_ID, node_order),),
        obligation="requirement",
...
@pytest.fixture
def hf_fragment(draft_with_supply_fragments: ImportedRuleDraft) -> RawClauseFragment:
    return _fragment_of(draft_with_supply_fragments, HF_FRAGMENT_ID)
...
def completed_system_voltage_draft(system_voltage_draft: ImportedRuleDraft) -> ImportedRuleDraft:
    """Two authored statements on one route: one citing node 0, one citing node 2.

    Authored out of statement order on purpose, so resolution has something to sort.
    """

    draft = author_clause_fact(
        system_voltage_draft,
        rule_route=SV_ROUTE,
        fact=_system_voltage_fact(system_voltage_draft, statement_index=1, node_order=2),
        actor="tester",
...
def test_a_route_without_authored_facts_refuses(
    draft_with_supply_fragments, hf_spec, hf_fragment
) -> None:
    """Projecting a branch nobody reviewed is what this slice exists to stop."""

    with pytest.raises(ClauseFactResolutionError):
        resolve_confirmed_clause_facts(hf_spec, hf_fragment, draft_with_supply_fragments)
...
def test_a_fragment_that_gained_a_node_reopens_the_routes_completion(
    ...
    with pytest.raises(ClauseFactResolutionError, match="older fragment"):
        resolve_confirmed_clause_facts(hf_spec, grown, draft)
...
def test_facts_come_back_ordered_by_statement_index(completed_system_voltage_draft) -> None:
    ...
    facts = resolve_confirmed_clause_facts(
        spec,
        _fragment_of(completed_system_voltage_draft, SV_FRAGMENT_ID),
...
def test_the_legacy_branch_authority_route_resolves_to_nothing(
    ...
    spec = _spec(PROPAGATION_ROUTE)
    fragment = _fragment_of(draft_with_supply_fragments, f"raw-{PROPAGATION_ROUTE}")

    facts = resolve_confirmed_clause_facts(spec, fragment, draft_with_supply_fragments)
